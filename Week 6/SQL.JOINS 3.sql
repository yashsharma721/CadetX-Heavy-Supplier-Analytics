...
SELECT c.customer_id,SUM(s.grand_total) AS Total_Sales FROM customers c JOIN sales_orders_header s ON c.customer_id = s.customer_id GROUP BY customer_id ORDER BY Total_Sales DESC LIMIT 10;
SELECT c.customer_id,COUNT(s.so_id) AS Total_Orders FROM customers c JOIN sales_orders_header s on c.customer_id = s.customer_id GROUP BY customer_id ORDER BY Total_Orders DESC LIMIT 10;
SELECT b.branch_name,SUM(s.grand_total) AS Total_Sales FROM branches b JOIN sales_orders_header s on b.branch_id = s.branch_id GROUP BY branch_name ORDER BY Total_Sales DESC LIMIT 10;
SELECT p.product_name,SUM(s.line_grand_total) AS Total_Revenue FROM products p JOIN sales_orders_lines s on p.product_id = s.product_id GROUP BY product_name ORDER BY Total_Revenue DESC LIMIT 10;
SELECT p.product_name,COUNT(s.quantity) AS Total_quantity FROM products p JOIN sales_orders_lines s on p.product_id = s.product_id GROUP BY product_name ORDER BY Total_Quantity DESC LIMIT 10; 
SELECT s.supplier_name,COUNT(p.po_id) AS Total_Orders FROM suppliers s JOIN purchase_orders_header p ON s.supplier_id = p.supplier_id GROUP BY supplier_name ORDER BY Total_Orders DESC LIMIT 10;

SELECT s.supplier_name,SUM(p.grand_total) AS Total_purchase_value FROM suppliers s JOIN purchase_orders_header p ON s.supplier_id = p.supplier_id GROUP BY supplier_name ORDER BY Total_Purchase_Value DESC LIMIT 10;
SELECT p.po_id,SUM(pl.line_grand_total) AS purchase_orders FROM purchase_orders_header p JOIN purchase_orders_lines pl ON p.po_id = pl.po_id GROUP BY po_id ORDER BY Purchase_Orders DESC LIMIT 10;
SELECT p.product_name,i.reorder_level,i.current_stock FROM products p JOIN inventory_master i ON p.product_id = i.product_id WHERE current_stock < reorder_level;
SELECT i.invoice_id,i.grand_total,p.payment_amount FROM invoices i JOIN payments p WHERE i.invoice_id = p.invoice_id ORDER BY payment_amount DESC LIMIT 10;
SELECT b.branch_name,SUM(i.current_stock) AS Total_Stock FROM branches b JOIN inventory_master i ON b.branch_id = i.branch_id GROUP BY branch_name ORDER BY Total_Stock DESC LIMIT 10; 
SELECT payment_method,SUM(payment_amount) FROM payments GROUP BY payment_method ORDER BY SUM(payment_amount) DESC LIMIT 10;

SELECT c.customer_id,i.grand_total AS Total_Invoice_Value FROM customers c JOIN invoices i ON c.customer_id = i.customer_id ORDER BY Total_Invoice_Value DESC LIMIT 10;
SELECT b.branch_name,i.grand_total AS Total_Invoice_Value FROM branches b JOIN invoices i ON b.branch_id = i.branch_id ORDER BY Total_Invoice_Value DESC LIMIT 10;
SELECT p.product_name,i.current_stock FROM products p JOIN inventory_master i ON p.product_id = i.product_id ORDER BY current_stock DESC LIMIT 10;
SELECT s.supplier_name,p.po_id,p.total_cost FROM purchase_orders_header p JOIN suppliers s ON p.supplier_id = s.supplier_id ORDER BY total_cost DESC LIMIT 10;
SELECT c.customer_id,s.so_id,s.grand_total FROM customers c JOIN sales_orders_header s ON c.customer_id = s.customer_id ORDER BY grand_total DESC LIMIT 10;
SELECT i.invoice_id,p.payment_amount FROM invoices i JOIN payments p ON i.invoice_id = p.invoice_id ORDER BY payment_amount DESC LIMIT 10;